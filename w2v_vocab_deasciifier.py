...
def deasciify(messages):
    deasciifier = Deasciifier(messages)
    messages = deasciifier.convert_to_turkish()
    return messages    

messages = [deasciify(m) for m in messages if str(m) != 'nan']

# filtered_messages = [re.sub(r"['].*?[^\w]", " " , row) for row in messages]
filtered_messages =[re.sub("[^a-zA-ZığüşöçİĞÜŞÖÇ ']"," ", re.sub(r"['].*?[^\w]", " " , row)) for row in messages]
...
with open('Documents/Deasciifier.txt', 'w', encoding='utf-8') as f:
